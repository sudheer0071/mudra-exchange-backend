...
//       console.log("inside the if 1 -/////////////", txHashs)
//       if (txHashs.transactions != null && txHashs.transactions.length != 0) {
//         console.log("inside the if 2 -/////////////", txHashs)
...
cron.schedule("* * * * * *", async () => {
  ...
  try {
    ...
    const txHashs = resp.data.result;
    console.log("networknetwork",network,"txHashstxHashs",txHashs?.transactions?.length)
    console.log("---network", network);
    if (txHashs != null && !isEmpty(txHashs.hash)) {
      console.log("inside the if 1 -/////////////", txHashs)
      if (txHashs.transactions != null && txHashs.transactions.length != 0) {
        console.log("inside the if 2 -/////////////")
        txHashList(txHashs.transactions, 0);
      }
      console.log("outside the if 1 -/////////////")
      network["block"] = network["block"] + 1;
      fs.writeFileSync("./config/network.json", JSON.stringify(network));
    }
  } catch (err) {
...
function txHashList(txHashs, index) {
  if (isEmpty(txHashs[index])) return;
  txDetail(txHashs[index]);
  txHashList(txHashs, index + 1);
}

async function txDetail(txData) {
  try {
    console.log("PROCESSING TRANSACTION:", txData.hash);
    console.log("TO:", txData.to, "FROM:", txData.from);
    console.log("checkContract:", checkContract, "checkUsrDeposit:", checkUsrDeposit);
    console.log("depositAddress includes to?", depositAddress.includes(txData.to.toLowerCase()));
    ...
    if (
      ...
      if (checkContract == true && !isEmpty(contractInfo)) {
        ...
        console.log("FOUND BNB DEPOSIT TO USER ADDRESS, CALLING WEBHOOK");
        webhookCall({
          address: txData.to,
        ...
        console.log("TRANSACTION DOESN'T MATCH CRITERIA FOR WEBHOOK");
      }
    }
    return true;
  } catch (err) {
    console.log("TRANSACTION PROCESSING ERROR:", err);
    hset("bnbTrxDetailError", txData.hash, err.toString());
    return false;
  }
}
...
async function webhookCall(reqBody) {
  try {
    console.log("ATTEMPTING WEBHOOK CALL WITH:", reqBody);
    ...
    console.log("WEBHOOK AUTH SUCCESS, MAKING API CALL");
    ...
    console.log("WEBHOOK RESPONSE:", respData.status, respData.data);
    return true;
  } catch (err) {
    console.log("WEBHOOK ERROR:", err.message, err.response?.data);
...
cron.schedule("* * * * *", async () => {
  console.log("----failer webhook cron", failWebhookCron);
  if (failWebhookCron) {
    return;
  }
  failWebhookCron = true;
...
const initialCall = async () => {
  ...
  try {
    ...
    console.log("service wallet url: ",config.SERVICE.WALLET.URL)
    let respData = await axios({
      method: "get",
      headers: {
        Authorization: auth.token,
...
(async function () {
  console.log("STARTING BNB NODE INITIALIZATION");
  let status = await initialCall();
  console.log("INITIALIZATION STATUS:", status);
  console.log("NETWORK:", network);
  console.log("DEPOSIT ADDRESSES COUNT:", depositAddress.length);
  console.log("CONTRACT ADDRESSES COUNT:", Object.keys(contractAddress).length);
  console.log("ADMIN ADDRESS:", adminAddress);
  let redisStatus = await redisConnection();
  console.log("REDIS CONNECTION:", redisStatus);
  console.log("CHECK USER DEPOSIT:", checkUsrDeposit);
  console.log("CHECK CONTRACT:", checkContract);
  
  runCron = false;
  failWebhookCron = false;
  app.listen(config.PORT, function () {
    console.log(`BNB NODE LISTENING ON PORT ${config.PORT}`);
  });
})();
