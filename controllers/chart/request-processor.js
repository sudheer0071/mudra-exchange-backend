...
function filterDataPeriod(data, fromSeconds, toSeconds) {
	...
	// console.log("--------fromSeconds",fromSeconds)

	if (!data || !data.t) {
		return data;
	}

	// console.log("-----data.t[data.t.length - 1] < fromSeconds",data.t[data.t.length - 1],fromSeconds,data.t[data.t.length - 1] < fromSeconds)
...
RequestProcessor.prototype._sendSymbolHistory = async function (symbol, startDateTimestamp, endDateTimestamp, resolution, response, tradeType) {
	...
	// let { chartData, callBy } = await chart({
	// 	pairName: pair,
	// 	timeType
	// });

	// console.log("tradeChart.callBytradeChart.callBytradeChart.callBy",tradeChart)
